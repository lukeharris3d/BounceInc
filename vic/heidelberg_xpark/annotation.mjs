...
export class Annotation extends Script {
  ...
  /** @type {number} */
  static hotspotSize = 30;

  /** @type {Color} */
  static hotspotColor = new Color(0.93, 0.16, 0.48);

  /** @type {Color} */
  static hoverColor = new Color(0.0, 0.67, 0.85);
  ...
  static _initializeStatic(app) {
    ...
    const createLayer = (name, semitrans) => {
      ...
      const idx = semitrans
        ? layers.getTransparentIndex(worldLayer)
        : layers.getOpaqueIndex(worldLayer);
  ...
  initialize() {
    ...
    this.app.on("prerender", () => {
      ...
      this.materials[1].setParameter(
        "material_opacity",
        0.25 * Annotation.opacity
      );
    });
  }
  ...
  setHover(hover) {
    this.materials.forEach((material) => {
      material.emissive.copy(
        hover ? Annotation.hoverColor : Annotation.hotspotColor
      );
      material.update();
    });
  ...
  _calculateScreenSpaceScale() {
    ...
    const worldSize =
      (Annotation.hotspotSize / screenHeight) *
      ((2 * distance) / projMatrix.data[5]);
